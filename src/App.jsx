...
═══════════════════════════════════════════════════════════════════ */
const SUPABASE_URL     = "https://gkcafcpkopuwdvbvvmuv.supabase.co";
const SUPABASE_ANON_KEY = "sb_publishable_6UWdUr_YymKM9KMhU0RjdQ_yygA4FUk";

/* Inline Supabase REST client (no npm needed in this environment) */
function mkSB(url, key) {
  const base = { "apikey":key, "Content-Type":"application/json", "Prefer":"return=representation" };
  const ah = (tok) => ({ ...base, "Authorization":"Bearer "+tok });
  const ah0 = base; // anon headers

  async function get(table, opts, tok) {
    let ep = url+"/rest/v1/"+table+"?";
    if(opts.select) ep += "select="+encodeURIComponent(opts.select)+"&";
    if(opts.eq) Object.entries(opts.eq).forEach(([k,v])=>{ ep += k+"=eq."+encodeURIComponent(v)+"&"; });
    if(opts.order) ep += "order="+opts.order+"&";
    if(opts.limit) ep += "limit="+opts.limit+"&";
    const r = await fetch(ep,{headers:tok?ah(tok):ah0});
    if(!r.ok) throw new Error(await r.text());
    return r.json();
  }
  async function post(table, data, tok) {
    const r = await fetch(url+"/rest/v1/"+table,{method:"POST",headers:tok?ah(tok):ah0,body:JSON.stringify(data)});
    if(!r.ok) throw new Error(await r.text());
    return r.json();
  }
  async function upsertRow(table, data, tok) {
    const hdrs = tok ? ah(tok) : ah0;
    hdrs["Prefer"] = "resolution=merge-duplicates,return=representation";
    const r = await fetch(url+"/rest/v1/"+table,{method:"POST",headers:hdrs,body:JSON.stringify(data)});
    if(!r.ok) throw new Error(await r.text());
    return r.json();
  }
  async function patch(table, data, eq, tok) {
    let ep = url+"/rest/v1/"+table+"?";
    Object.entries(eq).forEach(([k,v])=>{ ep += k+"=eq."+encodeURIComponent(v)+"&"; });
    const r = await fetch(ep,{method:"PATCH",headers:tok?ah(tok):ah0,body:JSON.stringify(data)});
    if(!r.ok) throw new Error(await r.text());
    return r.json();
  }
  async function callRpc(fn, params, tok) {
    const r = await fetch(url+"/rest/v1/rpc/"+fn,{method:"POST",headers:tok?ah(tok):ah0,body:JSON.stringify(params)});
    if(!r.ok) throw new Error(await r.text());
    return r.json();
  }
  const auth = {
    async signUp(email,pass) {
      const r = await fetch(url+"/auth/v1/signup",{method:"POST",headers:ah0,body:JSON.stringify({email,password:pass})});
      return r.json();
    },
    async signIn(email,pass) {
      const r = await fetch(url+"/auth/v1/token?grant_type=password",{method:"POST",headers:ah0,body:JSON.stringify({email,password:pass})});
      return r.json();
    },
    async signOut(tok) {
      await fetch(url+"/auth/v1/logout",{method:"POST",headers:ah(tok)});
    },
    async getUser(tok) {
      const r = await fetch(url+"/auth/v1/user",{headers:ah(tok)});
      return r.json();
    },
  };
  return { get, post, upsert:upsertRow, patch, rpc:callRpc, auth };
}
const sb = mkSB(SUPABASE_URL, SUPABASE_ANON_KEY);
...
const FM_NODES = {
  ...
  wait:{
    ...
    choices:[
      ...
      {id:"b",label:"Cut all non-essential costs immediately",desc:"Reduce staff hours, utilities. Buy a few more weeks while you plan.",impact:{cash:+120000,staffMorale:-20,ownerStress:+10},next:"cost_cut"},
    ],
  },
  cost_cut:{
            ...
            return(
              <tr key={ri} style={{background:flagged?"#FF525208":ri%2===0?"transparent":"#ffffff02"}}>
                {row.map((cell,ci)=>{
                  ...
                  const isBold=typeof cell==="string"&&cell===cell.toUpperCase()&&cell.length>3&&ci===0;
                  return <td key={ci} style={{padding:"8px 10px",textAlign:ci===0?"left":"right",color:isDeltaDown?T.red:isDeltaUp?T.green:isNeg?"#FF8C8C":ci===0?"#ccc":"#fff",fontWeight:isBold?700:ci===0?400:500,fontSize:isBold?11:12,borderBottom:`1px solid ${T.border}`,borderLeft:ci===0&&flagged?`3px solid ${T.red}`:ci===0?"3px solid transparent":"none"}}>{cell}</td>;
                })}
              </tr>
            );
          })}</tbody>
...
function AuthModal({onClose, onAuth}){
  ...
  async function handleSubmit(){
    ...
    try {
      if(mode==="signup"){
        if(!username.trim()){ setErr("Username is required"); setLoading(false); return; }
        const res = await sb.auth.signUp(email, pass);
        if(res.error){ setErr(res.error.message||"Sign up failed"); setLoading(false); return; }
        // Create profile row
        const userId = res.user?.id || res.identities?.[0]?.user_id;
        if(userId){
          await sb.upsert("profiles", {
            id: userId, username: username.trim(),
            xp: 0, rank: "SEED", cases_completed: 0,
            xp_gained_today: 0,
          }, res.access_token);
        }
        onAuth({ token: res.access_token, user: { id: userId, username: username.trim(), xp:0, rank:"SEED", cases_completed:0, xp_gained_today:0 }});
      } else {
        const res = await sb.auth.signIn(email, pass);
        if(res.error){ setErr(res.error.message||"Sign in failed"); setLoading(false); return; }
        // Fetch profile
        const profiles = await sb.get("profiles", { eq:{ id: res.user.id }, select:"*" }, res.access_token);
        const profile = profiles?.[0] || { id: res.user.id, username: email.split("@")[0], xp:0, rank:"SEED", cases_completed:0, xp_gained_today:0 };
        // Persist session
        localStorage.setItem("ca_token", res.access_token);
        localStorage.setItem("ca_user", JSON.stringify(profile));
        onAuth({ token: res.access_token, user: profile });
      }
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"#000000cc",display:"flex",alignItems:"center",justifyContent:"center",zIndex:999,animation:"fadeIn .2s both"}}
      ...
      <div style={{background:T.surf,border:`2px solid ${T.gold}`,padding:"40px 36px",width:380,animation:"fadeUp .25s both"}}
        ...
        <input value={pass} onChange={e=>setPass(e.target.value)} placeholder="Password" type="password" style={{...inp,marginBottom:0}}
...
export default function App(){
  ...
  async function fetchLeaderboard(){
    try{
      const data = await sb.get("profiles",{
  ...
  async function fetchFeed(){
    try{
      const data = await sb.get("activity_feed",{
  ...
  async function handleSignOut(){
    if(token) await sb.auth.signOut(token).catch(()=>{});
    clearSession();
  }
